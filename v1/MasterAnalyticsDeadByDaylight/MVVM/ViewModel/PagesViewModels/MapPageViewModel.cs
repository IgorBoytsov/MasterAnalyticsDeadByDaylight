            ...
            if (SelectedTypeMap == "Карты")
            {
                foreach (var map in _dataService.GetAllDataInList<Map>())
                {
                    ...
                    var mapStat = new MapStat
                    {
                        ...
                        PickRateMap = await CalculationMap.PickRateAsync(matches.Count, _dataService.Count<GameStatistic>(x => x.Where(x => x.IdKillerNavigation.IdAssociation == SelectedPlayerAssociation.IdPlayerAssociation))),
