...
{
    internal sealed class OfferingCategoryViewModel(OfferingCategoryResponse model) : ObservableObject<OfferingCategoryResponse>(model)
    {
        ...
        public override OfferingCategoryResponse ToModel()
            => _model with
            {
                OldId = this.OldId,
                Name = this.Name
